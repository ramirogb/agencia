...
if (!isset($_COOKIE['screenresolution'])) $_COOKIE['screenresolution']=1200;
...
$tipo=$_SESSION['tipe'][$i];	 //decia post no se por que
	   		 if ( $pasajeros >=1)
	   		{
...
//-->
    ...
    <span  style="padding-left:100px; margin-top:10px" class="minititle"><?php echo $select_catalog; ?><img src="images/plus.png" width="16" height="16" border="0">
    <?php ?>
    <?php 
  ...
  ?>
    ...
    <span ></span >&nbsp;<a href="<?php echo $_SERVER['PHP_SELF'] ?>?action=cotiza&m=1"><strong><?php echo$text_titleope; ?></strong></a>
    <a href="<?php echo $_SERVER['PHP_SELF'] ?>?action=reserv&m=2"><strong><?php echo $text_titlereq ?></strong></a>
   
    <p>&nbsp;</p>
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>?action=us" Method="post" enctype="multipart/form-data" name="forma" id="forma">
    <table   width="100%" align="<?php echo $maintablealign ?>" cellpadding="0" cellspacing="0" class="boxborder">
  ...
  <tr bgcolor="#EEEEEE">
    ...
    <td width="120" ><strong> <?php echo $hour_reserv ?></strong></td>
    <td colspan="2" ><strong>Cat</strong></td>
    <td width="1" ><div align="center"><strong><?php echo $text_titleclo
 ?></strong></div></td>
 ...
 ?></strong></div></td>
    ...
    <td width="20" ><div align="center"><strong><?php echo $comment
 ?></strong></div></td>
...
?>
 ...
 ?>        
      ...
      <td class="boxborder text"  style="padding-right:3px">
...
<option value="23"<?php     if (!(strcmp("23", $hor ) ) ) {echo "SELECTED"; } ?> ><?php echo '23h'; ?></option> 
      ...
      <td width="1" bgcolor="<?php echo UseColor() ?>"  style="padding-right:3px" ><a href="javascript:hideshow2(document.getElementById('adivCat2[<?php echo $j; ?>]'),<?php echo $j; ?>)"><img src="images/plus.png" width="16" height="16" border="0"></a>
      </td>
      <td width="1" bgcolor="<?php echo UseColor() ?>"  style="padding-right:1px" ><span  id="adivCat2<?php 
	  if ($_COOKIE['screenresolution']>1024) echo 'naa';
	      ?>[<?php echo $j; ?>]"; style="font:10px bold; <?php if ($_COOKIE['screenresolution']<1200) echo 'display:none'; ?> "   >
	    <select  class="comment3"  style="width:100px "   name="cate[<?php echo $j; ?>]" id="cate[<?php echo $j; ?>]" onChange="ajaxFunction('cate[<?php echo $j; ?>]');dowork('cate[<?php echo $j; ?>]','serv[<?php echo $j; ?>]','lev_cat');
	  hideshow2(document.getElementById('adivCat2[<?php echo $j; ?>]'),<?php echo $j; ?>)
	  ">
	...
	$row3x=query_db_assoc( $result577);	
			 do { ?>
        <option    value="<?php echo $row3x['tarif_cdg']; ?>&j=<?php echo $j; ?>" <?php 
			if (!(strcmp( trim($_SESSION['cate'][$j]), $row3x['tarif_cdg'] ) ) ) {echo "SELECTED"; 
			$_SESSION['cate'][$j]=$row3x['tarif_cdg']; }
			 ?>><?php echo  substr($row3x['descript'] ,0,50);  ?></option>
        <?php } while ($row3x=query_db_assoc(	$result577)); ?>
      </select></span></td>
      <td  ><div align="left">
        <select     style=" width:200px"   name="serv[<?php echo $j; ?>]"   class="comment3"     id="serv[<?php echo $j; ?>]"  onChange="ajaxFunction('serv[<?php echo $j; ?>]');dowork('serv[<?php echo $j; ?>]','tipe[<?php echo $j; ?>]','lev_service')" >
  ...
  {//==============  
	...
	$row3x=query_db_assoc( $result577);	
			 do { ?>
            <option    value="<?php echo $row3x['SerCdg']; ?>&j=<?php echo $j; ?>" <?php if (!(strcmp( trim($_SESSION['serv'][$j]), $row3x['SerCdg'] ) ) ) {echo "SELECTED"; } ?>><?php echo  $row3x['SerCdg'].' '.substr($row3x['SerDsc'] ,0,25);  ?></option>
            <?php } while ($row3x=query_db_assoc(	$result577)); ?>
          </select>
...
and tarifasdetalle.SerCdg='$tgt' and tarifasdetalle.Paxini='1'";	 
	...
	?>	  <td >
	<select  class="comment3" name="tipe[<?php echo $j; ?>]" onChange="ajaxFunction('tipe[<?php echo $j; ?>]');dowork('tipe[<?php echo $j; ?>]','lang[<?php echo $j; ?>]','lev_tipe')" id="tipe[<?php echo $j;
	   $ti=$_SESSION['tipe'][$j];
	   ?>]">	   
	 ...
	 $fila=query_db_assoc($resultsdd);
         ...
         do { ?>
        <option    value="<?php echo $fila['kind']; ?>&j=<?php echo $j; ?>"<?php 
		if (!(strcmp( trim($_SESSION['tipe'][$j]), $fila['kind'] ) ) ) 
		{echo "SELECTED"; }
		 ?> ><?php echo $fila['descript'];  ?></option>
...
?>
    <tr bgcolor="<?php echo UseColor() ?>">
      ...
      <td rowspan="3" class="boxborder">&nbsp;</td>
      <td rowspan="3" class="boxborder text">&nbsp;</td>
      <td rowspan="3" class="text"><input name="Clear"   style="margin:4px " type="submit"  id="Clear" value="<?php echo $calcule; ?>"></td>
...
?>
    ...
    <tr bgcolor="<?php echo UseColor() ?>">
      <td >        <div align="center">Total:        </div></td><td bgcolor="<?php echo UseColor() ?>" ><?php echo $ggx+$taxes;?>       
