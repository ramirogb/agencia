...
//==No login required
IF ($_GET['action']=='paypal_ipn')
{$authz='TRUE';
include('./includes/paypal_ipn_handler.php');
exit;
}
//=====
...
IF (  ($_GET['action']=='settings') )
{
  ...
  { //ONLY ADMIN	
   ...
   $c->config_set('retries',$_POST['retries']);   
	...
	$c->config_set('$paypal_email',$_POST['paypal_email']);
	$c->config_set('paypal',$_POST['paypal']);	
  	$c->config_closeFile();  
...
//=============LISTINGS
	...
	reserv_master.timestamp,they_owe_you
...
//=========================================
 ...
 quotation_master.timestamp,quotation_master.total_price as they_owe_you, 
...
//============================================
	...
	$_SESSION['el_service']=$ing[0]; //used for step2 creating a tarif
...
//==================
IF ($_GET['action']=='paypal_2') //redirects to sandbox,
{$authz='TRUE';
...
$_SESSION['payer_id']= $_POST['PayerID'];
		
include('./includes/top_quo.php');
include('./includes/paypal_confirm.php');
include('includes/bottom.php');
exit;
...
$id=$_GET['id'];
$_SESSION['reserva']=$id; 
if ( user_level2() ==0) {  $query="select * from reserv_master where DocNro='$id' and AgeCdg='$your_agency'";}
if (user_level2() >0)   {  $query="select * from reserv_master where DocNro='$id'";}

$result = query_db($query);
$row3x8=query_db_assoc( $result);	
 $_SESSION["Payment_Amount"]=$row3x8['they_owe_you'];
include('./includes/top_quo.php');
include('./includes/pay_pal1.php');
include('includes/bottom.php');
exit;
}
//==================
IF ($_GET['action']=='paypal_ok')
{$authz='TRUE';
$id=$_GET['id'];
include('./includes/top_quo.php');
include('./templates/paypal_ok.php');
include('includes/bottom.php');
exit;
}
//==================
IF ($_GET['action']=='paypal_err')
{$authz='TRUE';
$id=$_GET['id'];
include('./includes/top_quo.php');
include('./templates/paypal_err.php');
include('includes/bottom.php');
exit;
}
//==================
...
//Prepare vars for template
	...
	if ($reserva==1){
			...
			if ($paypal=='TRUE')
			{
			$dataText .=$invoice."<BR>$siteurl/quote.php?action=paypal&id=$CODIGO_DE_OPE";
			}
...
$co=1;$el_id=$_GET['id'];$fi=$_POST['firstname'][$co];$la=$_POST['last_name'][$co];$sex=$_POST['sex'][$co];$phone=$_POST['phone'][$co];$email=$_POST['email'][$co];
$id=$_POST['id']; $updateSQL ="UPDATE passengers  SET FirstName ='$fi', LastName ='$la',Gender='$sex',phone1='$phone',email='$email' where Id='$id'";
if ( ($id=='') or ($id==0))
{
$doc=$_POST['reserv'];
 $updateSQL = "INSERT INTO passengers	SET FirstName ='$fi', LastName ='$la',Gender='$sex',phone1='$phone',id_reserv='$doc',email='$email'  ";
}
...
include('includes/bottom.php');
log_booking($id,"updated Pax:$id FirstName ='$fi',phone=$phone,LastName ='$la");	
exit;
}
//+++++
begin_trans();
...
if ( $_GET['action']=='update_amount')
	{
	$add_amount=$_GET['add_amount'];$add_type=$_GET['add_type'];$add_comments=$_GET['add_comments'];$id=$_GET['id'];
	$query578 = "SELECT * from reserv_master WHERE DocNro='$id' ";
	$result578 = query_db($query578);$row3x8=query_db_assoc( $result578);$total_price=$row3x8['they_owe_you'];
